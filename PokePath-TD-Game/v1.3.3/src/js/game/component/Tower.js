...
export class Tower extends Sprite {
    ...
    setTowerStats() {
        ...
        if (this.pokemon?.item?.id == 'spindaCocktail') this.range = this.pokemon.range * 1.25;
    ...
    recalculatePower() {
        ...
        if (this.pokemon?.item?.id == 'choiceScarf') {
            if (this.ability.id === 'quadraShot') this.speed -= (this.speed * 0.6);
            else if (this.ability.id === 'tripleShot') this.speed -= (this.speed * 0.4);
            else this.speed -= (this.speed * 0.2);
        }
    ...
    draw() {
        ...
        if (this.powerAura || this.criticalAura || this.triageAura) {
            ...
            // Definimos combinaciones de auras y sus colores
            const auraColors = {
            ...
            // Construimos la clave según las auras activas
            let key = [];
        ...
        if (this.pokemon.adn != undefined) {
           // ---- CANVAS TEMPORAL ----
            ...
            if (this.pokemon.isShiny) {
                tctx.fillStyle = "rgba(100, 180, 255, 0.65)"; // azul suave
            } else {
                tctx.fillStyle = "rgba(255, 100, 150, 0.6)"; // rosa/morado
            }
        ...
        if (this.pokemon.attackType === 'area') {
            if (validEnemies.length > 0 && this.attackCooldown <= 0) {
                ...
                validEnemies.forEach(enemy => {
                    ...
                    if (this.pokemon?.item?.id == 'blueBandana') finalDamage += Math.floor(finalDamage * ((this.critical * 1.75) / 100));
