...
const TARGET_MODES_TRADUCTIONS = {
	...
	highHP: ['High HP', 'Mas PS', 'Le plus de vie', 'HP alto', 'Alta salute', 'Hohe KP', '高HP', '높은 HP', '高生命值'],
	lowHP: ['Low HP', 'Menos PS', 'Le moins de vie', 'HP baixo', 'Bassa salute', 'Niedrige KP', '低HP', '낮은 HP', '低生命值'],
	...
	burned: ['Burned', 'Quemado', 'Brûlé', 'Queimado', 'Scottato', 'Verbrannt', '火傷', '화상', '灼伤'],
	notBurned: ['Not Burned', 'No quemado', 'Non brûlé', 'Não queimado', 'Non scottato', 'Nicht verbrannt', '未火傷', '비화상', '未灼伤'],

	stuned: ['Stunned', 'Aturdido', 'Étourdí', 'Atordoado', 'Stordito', 'Betäubt', '気絶', '기절', '眩晕'],
	...
	nightmared: ["Nightmare'd", "Con pesadilla", "Avec cauchemar", "Com pesadelo", "Con incubo", "Mit Albtraum", "悪夢を伴う", "악몽을 동반한", "带着噩梦的"],
...
export class PokemonScene extends GameScene {
	...
	render() {
		...
		this.itemIcon.addEventListener('click' , () => this.itemWindow.open(this.pokemon));
		...
		this.buttonDNA.addEventListener('click', () => {
			...
			// 	console.log(pokemon)
			// 	//if(pokemon.tiles.includes(this.main.area.placementTiles[this.pokemon.tilePosition].land))
	...
	update() {
		...
		if (this.pokemon.id == 64 || this.pokemon?.adn?.id == 64) {
			...
			this.abilityDescription.innerHTML += `<br><br> * ${text.pokemon.current[this.main.lang]} -${this.main.player.fossilInTeam * 0.5}s`;
		} else if (
		...
		if (this.pokemon?.item?.id == 'quickClaw') {
			this.data['attackType'].style.pointerEvents = 'none';
			this.data['attackType'].style.opacity = '80%';
...
class ItemWindow {
	...
	update() {
		...
		this.main.player.items.forEach((item, i) => {
			...
			if (!able) this.slot[i].style.filter = 'grayscale(100%) drop-shadow(0 0 2px red) brightness(0.3)';
